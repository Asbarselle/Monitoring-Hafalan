...
class AudioAnalysisService
{
    ...
    /**
* Analyze audio file dari local storage path
 */
    public function analyzeAudio(string $audioPath): array
    {
        try {
            if (!Storage::disk('public')->exists($audioPath)) {
                throw new \Exception('Audio file not found in public storage: ' . $audioPath);
            }

            $tempPath = Storage::disk('public')->path($audioPath);
